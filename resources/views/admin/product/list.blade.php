...
<script>
    function ajax_product_active_inactive(Action,ProductId)
    {
        if(Action != "" && ProductId !="")
        {
            $.ajax({
                type: "POST",
                url: "<?php echo url('/admin/product/ajax_product_active_inactive'); ?>",
                data: {"_token": "{{ csrf_token() }}",Action: Action, ProductId : ProductId},
                dataType: 'text',
...
<div class="form-group">
    <div class="row"><!-- row START -->
        <div class="col-lg-12">
            ...
            <a href="<?php echo url('/admin/product/add_select_type_product'); ?>" class="btn btn-primary btn-quirk btn-stroke pull-right">Add Product</a>
            <?php } ?>
        </div>
    </div><!-- row END -->
</div>
                        ...
                        <table class="table table-bordered" id="example">
                            ...
                            <tr >
                                ...
                                <td><?php if($product['product_image']!='')
                                    { ?>
                                    <img src="<?php echo url('public/upload/product/thumb')?><?php echo '/'.$product['product_image'];?>" width="100" height="100" align="left"/>
                                    <?php
                                    }
                                    else
                                    { ?>
                                    <img src="<?php echo url('public/upload/product/thumb/noPreview.png');?>" width="100" align="left"/>
                                    <?php
                                    }?>
                                </td>
                                ...
                                <td>
                                    <a title="View Product" href="<?php echo url('admin/product/view'); ?><?php echo '/'.$product['product_id']; ?>"><i class="fa fa-eye fa-2x" aria-hidden="true"></i></a>
                                    <?php if(Session::get('admin_sess_is_admin')==1){ ?>
                                    <a title="Edit Product"  href="<?php echo url('admin/product/edit'); ?><?php echo '/'.$product['product_id']; ?>"><i class="fa fa-pencil fa-2x" aria-hidden="true"></i></a>&nbsp;
                                    <?php } ?>
                                    <a title="Gallery Product"  href="<?php echo url('admin/gallery/list'); ?><?php echo '/'.$product['product_id']; ?>"><i class="fa fa-camera fa-2x" aria-hidden="true"></i></a>&nbsp;
                                    <?php if(Session::get('admin_sess_is_admin')==1){ ?>
                                    <a title="Delete Product"  href="#" onclick="javascript : deleteConfirm('<?php echo url('admin/product/delete'); ?><?php echo '/'.$product['product_id']; ?>')" style="text-decoration:none;"><i class="fa fa-trash fa-2x" aria-hidden="true"></i></a>&nbsp;
                                    <?php } ?>
                                    <?php
