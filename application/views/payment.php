...
<script>
    $(document).ready(function(){
        $('.final_checkout').click(function(){
            $.ajax({
                type: "POST",
                url: "<?php echo site_url('cart/ajax_confirm_order'); ?>",
                data: {},
                dataType: 'text',
    ...
    <div class="row" style="margin-top: 5%;">
        <div class="col-sm-12">
            <table class="table" style="border-top:1px solid black; ">
                <thead>
                <tr>
                    <th width="25%" class="before_active"><center><a href="<?php echo base_url();?>cart/shopping_cart">Summary</a></center></th>
                    <th width="25%" class="before_active"><center><a href="<?php echo base_url();?>cart/delivery_deatil">Delivery Deatil</a></center></th>
...
{?>
    <div class="row">
        ...
        <div class="col-xs-3" style="border: 2px solid #CCCCCC;">
             ...
             <div class="row">
                <div class="col-sm-12"><p><?php echo $deliveryDetails[0]['user_address']; ?></p></div>
             </div>
        </div>
    </div>
   <!-- start footer -->
            ...
            <div style="float:right;">
					  <?php if($this->session->userdata('front_logged_user_in')==1)
                      {
                        ...
                        <form action="<?php echo $paypal_url; ?>" method="post" name="frmPayPal" id="frmPayPal">
                            ...
                            <?php $i++; } ?>
                            <input type="hidden" name="shipping_1" value=" <?php echo shipping_calculate(sub_total($this->session->userdata('front_user_id'))); ?>">
                            ...
                            <input type="hidden" name="cancel_return" value="<?php echo $this->config->item('CLOTHESLOOP_ROOT_FOLDER'); ?>/cart/cancel_payment">
                            <input type="hidden" name="return" value="<?php echo $this->config->item('CLOTHESLOOP_ROOT_FOLDER'); ?>/cart/success_payment">
